...
Bring-your-own-key and local-model AI configuration: which providers exist,
where paper content is sent, how routing tiers map to models, and how
failures degrade. (Base v1 requirements — BYO keys, keychain storage,
per-action routing, no-key mode, privacy boundary — merge in when the
`add-v1-foundation` change archives; the requirements below were added by
`add-zai-glm-provider`.)
